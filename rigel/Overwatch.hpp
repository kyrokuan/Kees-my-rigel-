...
namespace OW {
			...
			*/
			...
			while (FindWindowA(skCrypt("TankWindowClass"), NULL))
			{
				...
				if (Config::Menu)
				{
					...
					if (ImGui::Begin(skCrypt(u8"Kees"), &_visible, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoTitleBar)) {
						ImGui::Spacing();
						ImGui::PushFont(ico);
	...
	//
									...
									if (ImGui::Button(skCrypt(u8"Click to set keybind")))
									{
										capturingKey = true;
									}
									if (capturingKey)
									...
									//Attempt fixing later
								//	for (int button = 0; button < 5; button++) // Assuming 5 mouse buttons
								//	{
								//		if (ImGui::Selectable(key_type[button], type)
								//		{
								//			keys = key_type[button]
								//			switch (button)
								//			{
								//			case 0:
								//				Config::aim_key = VK_LBUTTON;
								...
								//				break;
								//			case 3:
								//				Config::aim_key = VK_XBUTTON1; // For side button 1
								//				break;
								//			case 4:
								//				Config::aim_key = VK_XBUTTON2; // For side button 2
								//				break;
								//			default:
							...
							if (subindex == 3) {
								...
								ImGui::Toggle(skCrypt(u8"Acceleration 2"), &Config::acctoggle, ImGuiToggleFlags_Animated);
								ImGui::SliderFloat(skCrypt(u8"Tracking Acceleration"), &Config::trackacc, 0.f, 1000.f, skCrypt("%.2f"));
					...
					if (Config::Tracking) {
						...
						while (GetAsyncKeyState(Config::aim_key) && !Config::reloading)
						{
							...
							if (vec != Vector3(0, 0, 0) && !(entities[Config::Targetenemyi].skill2act && entities[Config::Targetenemyi].HeroID == eHero::HERO_GENJI) && ((!entities[Config::Targetenemyi].imort && !entities[Config::Targetenemyi].barrprot) || Config::switch_team)) {
								...
								auto calc_target = CalcAngle(XMFLOAT3(vec.X, vec.Y, vec.Z), viewMatrix_xor.get_location());
								auto vec_calc_target = Vector3(calc_target.x, calc_target.y, calc_target.z);
								auto Target = TrackingAcceleration(local_angle, vec_calc_target, Config::Tracking_smooth / 10.f, Config::trackacc);
			...
			if (!Config::Menu) {
				if (Config::lastheroid != local_entity.HeroID) {
					if (Config::lastheroid != 0) {
						...
						_stprintf(bufsave, _T("%d"), Config::acctoggle);
						WritePrivateProfileString(_T(GetHeroEngNames(Config::lastheroid, local_entity.LinkBase).c_str()), _T("acctoggle"), bufsave, _T(".\\config.ini"));
						_stprintf(bufsave, _T("%d"), (int)Config::trackacc * 10000);
						WritePrivateProfileString(_T(GetHeroEngNames(Config::lastheroid, local_entity.LinkBase).c_str()), _T("trackacc"), bufsave, _T(".\\config.ini"));
					...
					Config::trackacc = float(GetPrivateProfileInt(_T(GetHeroEngNames(local_entity.HeroID, local_entity.LinkBase).c_str()), _T("comspeed"), 5000, _T(".\\config.ini"))) / 10000.f;
