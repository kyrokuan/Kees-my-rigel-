...
namespace OW {
	...
	//New acceleration test starts here
	//////////////////////////////////////
	// Works wonders, I have to get it to work with Config::comspeed for options and tracking speed to make it less blatant
	////////////////////////////
	/*
	inline Vector3 SmoothLinear(Vector3 LocalAngle, Vector3 TargetAngle, float speed) {
		float dist = LocalAngle.DistTo(TargetAngle);
		static Vector3 lastDelta{ 0, 0, 0 };

		Vector3 Result;

		// Calculate deltas with acceleration factor
		Vector3 delta;
		delta.X = (TargetAngle.X - LocalAngle.X);
		delta.Y = (TargetAngle.Y - LocalAngle.Y);
		delta.Z = (TargetAngle.Z - LocalAngle.Z);

		// Apply an acceleration factor based on distance and previous speed
		float accelFactor = dist * 0.1f; // Adjust the factor based on your needs
		Vector3 acceleration;
		acceleration.X = (delta.X - lastDelta.X) * accelFactor;
		acceleration.Y = (delta.Y - lastDelta.Y) * accelFactor;
		acceleration.Z = (delta.Z - lastDelta.Z) * accelFactor;

		// Update deltas with acceleration
		delta.X += acceleration.X;
		delta.Y += acceleration.Y;
		delta.Z += acceleration.Z;

		// Speed limit
		delta.X = std::min(delta.X, speed);
		delta.Y = std::min(delta.Y, speed);
		delta.Z = std::min(delta.Z, speed);

		Result = LocalAngle + delta;
		lastDelta = delta; // Store current delta for the next frame
	...
	*/
	...
	inline Vector3 TrackingAcceleration(Vector3 LocalAngle, Vector3 TargetAngle, float Speed, float Acc)
	{
		...
		if (Config::acctoggle) {
			__try
			{
				...
				float length = delta.get_length();
				if (length < 0.0001f) return TargetAngle;

				// Ensure Speed and Acc values are within reasonable limits
				Speed = std::clamp(Speed, 0.0f, 1.0f);
				Acc = std::clamp(Acc, 0.0f, 10.0f);

				float tmp = Acc / length;
				tmp = tmp * tmp * 0.005f;
				...
				// Clamp the final value of c to be within [0, 1]
				if (c >= 1) c = 1;
				...
				// Debug output
				std::cout << "LocalAngle: " << LocalAngle.X << ", " << LocalAngle.Y << ", " << LocalAngle.Z << std::endl;
				std::cout << "TargetAngle: " << TargetAngle.X << ", " << TargetAngle.Y << ", " << TargetAngle.Z << std::endl;
				std::cout << "Delta: " << delta.X << ", " << delta.Y << ", " << delta.Z << std::endl;
				std::cout << "Speed: " << Speed << ", Acc: " << Acc << ", c: " << c << std::endl;
	...
	inline Vector3 SmoothLinear(Vector3 LocalAngle, Vector3 TargetAngle, float speed)
	{
		...
		// Update the local angle without overshooting
		if (abs(deltax) >= abs(TargetAngle.X - LocalAngle.X)) {
			Result.X = TargetAngle.X;
		}
		else {
			Result.X = LocalAngle.X + deltax;
		}

		if (abs(deltay) >= abs(TargetAngle.Y - LocalAngle.Y)) {
			Result.Y = TargetAngle.Y;
		}
		else {
			Result.Y = LocalAngle.Y + deltay;
		}

		if (abs(deltaz) >= abs(TargetAngle.Z - LocalAngle.Z)) {
			Result.Z = TargetAngle.Z;
		}
		else {
		...
		if (Config::trackcompensate) {
			...
			if (Result.DistTo(realresult) <= Config::comarea) {
				...
				if (abs(deltay) < abs(TargetAngle.Y - lasty)) deltay = (TargetAngle.Y - lasty) * Config::comspeed;
			...
			// Update the local angle again after compensation
			if (abs(deltax) >= abs(TargetAngle.X - LocalAngle.X)) {
				Result.X = TargetAngle.X;
			}
			else {
